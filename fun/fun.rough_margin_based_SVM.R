fun.rough_margin_based_SVM <- function(data, best){
    ...
    train <- train[order(-train$`0/1`, train$Class),]
    rm(prediction)
    
    source("./fun/fun.calc_rough_margin.R")
    ...
    test <- test[order(-test$`0/1`, test$Class),]
    print(table(test$Class, test$prediction.svm))
    print(table(test$Class, test$prediction.rough_margin, useNA = "ifany"))
